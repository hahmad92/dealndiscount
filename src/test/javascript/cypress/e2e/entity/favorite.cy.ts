...
describe('Favorite e2e test', () => {
  ...
  const favoriteSample = { userId: 'bashfully judicious sharp', favoriteType: 'STORE' };
  ...
  describe('new Favorite page', () => {
    ...
    it('should create an instance of Favorite', () => {
      cy.get(`[data-cy="userId"]`).type('abaft perambulate');
      cy.get(`[data-cy="userId"]`).should('have.value', 'abaft perambulate');

      cy.get(`[data-cy="storeId"]`).type('pfft');
      cy.get(`[data-cy="storeId"]`).should('have.value', 'pfft');

      cy.get(`[data-cy="cityId"]`).type('guess unimpressively');
      cy.get(`[data-cy="cityId"]`).should('have.value', 'guess unimpressively');

      cy.get(`[data-cy="cardId"]`).type('although');
      cy.get(`[data-cy="cardId"]`).should('have.value', 'although');

      cy.get(`[data-cy="favoriteType"]`).select('STORE');
